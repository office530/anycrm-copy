    ...
    return (
        <div className={cn("space-y-4", className)}>
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                activeView === view.id
                                    ...
                                    : theme === 'dark'
                                        ? "bg-muted text-muted-foreground border-transparent hover:bg-card hover:text-foreground"
            ...
            <div className={cn(
                ...
                theme === 'dark' 
                    ? "bg-card/80 border-border/50 shadow-black/20" 
                    : "bg-white/80 border-white/50 shadow-slate-200/50"
            )}>
                
                {/* Search Input */}
                <div className="relative flex-1 w-full md:w-auto min-w-[200px]">
                    <Search className={cn("absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4", theme === 'dark' ? "text-muted-foreground" : "text-slate-400")} />
                    <Input 
                        placeholder="Search..." 
                        value={search}
                        ...
                        className={cn(
                            ...
                            theme === 'dark' ? "bg-transparent text-foreground placeholder:text-muted-foreground" : "bg-transparent text-slate-900 placeholder:text-slate-400"
                        )}
                    />
                </div>

                {/* Vertical Divider */}
                <div className={cn("hidden md:block w-px h-6 mx-2", theme === 'dark' ? "bg-border" : "bg-slate-200")} />
                ...
                <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                            ...
                            <Popover key={field.key}>
                                <PopoverTrigger asChild>
                                    <div className={cn(
                                        ...
                                        theme === 'dark' 
                                            ? "bg-muted border-border text-primary hover:border-primary/50" 
                                            : "bg-slate-100 border-slate-200 text-slate-700 hover:border-blue-300 hover:bg-blue-50"
                                    )}>
                                        <span className={theme === 'dark' ? "text-muted-foreground" : "text-slate-500"}>{field.label}:</span>
                                        <span className="max-w-[100px] truncate">{displayValue}</span>
                                        <div 
                                            role="button"
                                            onClick={(e) => { e.stopPropagation(); handleRemoveFilter(field.key); }}
                                            className={cn("ml-1 p-0.5 rounded-full", theme === 'dark' ? "hover:bg-card" : "hover:bg-slate-200")}
                                        >
                                            <X className="w-3 h-3" />
                                        </div>
                                    </div>
                                </PopoverTrigger>
                                <PopoverContent align="start" className={cn("p-0 w-auto border shadow-xl", theme === 'dark' ? "bg-card border-border" : "bg-white")}>
                    ...
                    <Popover open={openPopover === 'add'} onOpenChange={(open) => setOpenPopover(open ? 'add' : null)}>
                        ...
                        <PopoverContent align="start" className={cn("p-1 w-48 shadow-xl", theme === 'dark' ? "bg-card border-border" : "bg-white")}>
                            <div className="space-y-1">
                                <div className={cn("px-2 py-1.5 text-xs font-semibold", theme === 'dark' ? "text-muted-foreground" : "text-slate-500")}>
                                    Filter by...
                                </div>
                                    ...
                                    availableFields.map(field => (
                                        <div 
                                            ...
                                            className={cn(
                                                ...
                                                theme === 'dark' ? "hover:bg-muted text-foreground" : "hover:bg-slate-100 text-slate-700"
                                            )}
                                        >
