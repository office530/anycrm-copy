        ...
        <div className="flex flex-col h-full relative">
            ...
            <div className={`p-6 lg:p-6 border-b flex flex-col items-center lg:items-start mt-10 lg:mt-0 ${theme === 'dark' ? 'border-slate-700' : 'border-neutral-50'}`}>
                <Link to={createPageUrl('Dashboard')} onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-3 text-2xl lg:text-2xl font-bold tracking-tight hover:opacity-80 transition-opacity">
                    {branding.logoUrl ? (
                        <img src={branding.logoUrl} alt="Logo" className={`w-16 h-16 lg:w-10 lg:h-10 object-contain rounded-xl p-1 ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`} />
                    ) : (
                        <div className={`rounded-xl lg:rounded-lg p-4 lg:p-2 shadow-lg ${theme === 'dark' ? 'bg-cyan-500 shadow-cyan-500/50' : 'bg-red-700 shadow-red-900/20'}`}>
                ...
                <p className={`text-sm mt-2 font-medium tracking-wide ${theme === 'dark' ? 'text-cyan-400' : 'text-neutral-500 opacity-80'}`}>OLD SALES DATABASE</p>
            </div>
